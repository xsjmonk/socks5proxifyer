...
- **UI payload or path rejected:** when an ACL check causes the rejection, review the launcher's **Verification details** for the exact rejected path, trustee SID, ACE mask, prohibited rights, and whether the ACE was inherited. A standard user having generic all/write, delete/delete-child, or permission/owner-control access to any installation-path ancestor, including `Program Files`, allows path substitution before elevation and must not be bypassed. Have the system administrator restore that Windows directory from the correct operating-system or organizational ACL baseline; do not recursively reset or copy permissions across `Program Files`.
...
- **Windows 7 UI startup failure:** use the current setup build and verify Windows 7 SP1 has current servicing-stack, SHA-2, and .NET Framework 4.7.2 updates. Builds must not call `SetDefaultDllDirectories` before WinForms starts on Windows 7 because an operating-system GDI+ bug then prevents even installed fonts from being constructed. The native host uses the compatible DLL-search policy described above and the managed UI retains its named/GDI font recovery for independently damaged font installations.
